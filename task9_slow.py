...
def jacobi_cupy_slow(u, interior_mask, max_iter):
    ...
    for i in range(max_iter):
        ...
        next_grid[1:-1, 1:-1][mask] = u_new[mask]
...
if __name__ == '__main__':
    ...
    jacobi_cupy_slow(all_u0[0], all_interior_mask[0], 10)

    start = time.perf_counter()
    all_u = np.empty_like(all_u0)
    for i, (u0, interior_mask) in enumerate(zip(all_u0, all_interior_mask)):
        u = jacobi_cupy_slow(u0, interior_mask, MAX_ITER)
        all_u[i] = u
